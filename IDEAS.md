...
| P6 | **Réseau** 🔨 (fait : `ping`, `nslookup`, `netstat`, `whois` — ABI v43 ; serveur **`ftpc`** ; reste : client FTP/FTPS dans le VFS) | **Serveur FTP** (déploiement à chaud), multi-utilisateurs sans fichier de config : `ftpc [homedir] [user] [password]` ; si une instance tourne déjà, la nouvelle informe seulement l'instance principale (IPC) « tel user, tel mot de passe, tel dossier racine » puis quitte. **Client FTP intégré au VFS** (un préfixe de chemin type `FTP:hôte/…` routé vers un client FTP, pour que File Viewer & co y naviguent comme sur la carte). `/bin/ping` (Circle répond déjà aux pings), `nslookup`, `netstat`, `whois`. | 💡 |
