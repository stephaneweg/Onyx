...
public:
	...
	void onDraw () override
	{
		...
		{
			const unsigned *s = ec_front (&g_ec) + y * gb::W;
...
static void blit_full (void)				// stretched to the display, proportions kept, centred
{
	...
	for (int y = 0; y < oh; y++)
	{
		...
		const unsigned *s = ec_front (&g_ec) + sy * gb::W;
...
static void palette (const unsigned *p) { ec_hold (&g_ec); g_m->setDmgPalette (p); ec_resume (&g_ec); }
static void on_green () { palette (PAL_GREEN); }
static void on_grey () { palette (PAL_GREY); }
static void on_pocket () { palette (PAL_POCKET); }
...
static void gb_frame (EmuCore *ec)
{
	...
	for (int i = 0; i < gb::W * gb::H; i++) d[i] = s[i];
	ec_publish (ec);
