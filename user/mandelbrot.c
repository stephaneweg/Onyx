...
#define FB	24
#define FONE	(1LL << FB)
static i64 fmul (i64 a, i64 b) { return (a * b) >> FB; }	// |a|,|b| < ~2^31 -> safe in i64
...
{
	if (it >= MAXITER) return 0x00000000;			// inside the set
...
static void render (void)
{
	...
	kapi_draw_text (6, RH + 1, "click: zoom in   o: out   r: reset", 0x0090b0a0);
	g_dirty = 0;
}
...
static void on_key (unsigned long s, int ev, long key)
{
	...
	if (key == 'o' || key == 'O') { g_span *= 2; g_dirty = 1; }
