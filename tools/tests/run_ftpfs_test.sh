...
#   2. our own ftpc (built like run_ftpc_test.sh), port 21 (needs root)
# TLS is stubbed out here (FTPS is exercised on hardware only).
set -e
here=$(cd "$(dirname "$0")" && pwd)
b=$(mktemp -d)
root=$(mktemp -d)
cleanup () { kill $srv 2>/dev/null || true; kill $ftpc 2>/dev/null || true; pkill -P $ftpc 2>/dev/null || true; rm -rf "$b" "$root"; }
trap cleanup EXIT
...
# 2. our ftpc (its mock maps SD:/ to /tmp/onyx_mock)
echo "--- against ftpc"
...
cp "$b/ftpc.c" "$b/srv/"
printf '#include "ftpc.c"\nint main (int c, char **v) { snprintf (mock_args, sizeof mock_args, "%%s", c > 1 ? v[1] : ""); return ftpc_main (); }\n' > "$b/srv/main.c"
g++ -w -I"$b/srv" -x c++ "$b/srv/main.c" -o "$b/ftpc"
rm -rf /tmp/onyx_mock && mkdir -p /tmp/onyx_mock/pub/sub && printf 'hello onyx\n' > /tmp/onyx_mock/pub/readme.txt
"$b/ftpc" "SD:/pub tester secret" > /dev/null & ftpc=$!
sleep 0.5
