...
static void fetch_file_process(struct fetch_file_context *ctx)
{
	...
	int onyx_sr = stat(ctx->path, &fdstat);	/* DEBUG */
	printf("ONYX-FILE path=%s stat=%d mode=0%o reg=%d dir=%d type=%s\n",
	       ctx->path, onyx_sr, onyx_sr == 0 ? (unsigned)fdstat.st_mode : 0u,
	       onyx_sr == 0 ? S_ISREG(fdstat.st_mode) : -1,
	       onyx_sr == 0 ? S_ISDIR(fdstat.st_mode) : -1,
	       guit->fetch->filetype(ctx->path));
	fflush(stdout);
