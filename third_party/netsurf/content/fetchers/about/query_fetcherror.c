...
bool fetch_about_query_fetcherror_handler(struct fetch_about_context *ctx)
{
	...
	printf("ONYX-FETCHERROR site=%s reason=%s\n",
	       siteurl ? nsurl_access(siteurl) : "(null)", reason ? reason : "(null)");
	fflush(stdout);	/* DEBUG: what fetch failed (root cause of the error-page loop) */
