...
nserror fetcher_init(void)
{
	...
	ret = fetch_onyx_register();		/* Onyx http:// fetcher */
	if (ret != NSERROR_OK) {
		return ret;
	}
	    ...
	    struct fetch **fetch_out)
{
	...
	/* DEBUG trace: see fetch_send_callback (failures only) + query_fetcherror */
...
fetch_send_callback(const fetch_msg *msg, struct fetch *fetch)
{
	...
	if (msg->type == FETCH_ERROR || msg->type == FETCH_TIMEDOUT ||
	    msg->type == FETCH_AUTH || msg->type == FETCH_CERT_ERR ||
	    msg->type == FETCH_SSL_ERR) {	/* DEBUG: only FAILED fetches */
		printf("ONYX-FETCH FAIL %s (type %d) err=%s\n",
		       fetch->url ? nsurl_access(fetch->url) : "?", msg->type,
		       (msg->type == FETCH_ERROR && msg->data.error) ? msg->data.error : "");
		fflush(stdout);
	}
